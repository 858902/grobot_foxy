...
from grobot_interfaces.srv import Battery
from grobot_interfaces.srv import Color
from grobot_interfaces.srv import SaveColor
from grobot_interfaces.srv import ResetOdom
from grobot_interfaces.srv import Onoff
...
class GrobotNode(Node):
  def __init__(self):
    ...
    _port_name = self.get_parameter_or('port.name', Parameter('port.name', Parameter.Type.STRING, '/dev/ttyMotor')).get_parameter_value().string_value # 미니는 ttyTHS1
    ...
    self.max_ang_vel_z = self.get_parameter_or('/motor/max_ang_vel_z', 
                Parameter('/motor/max_ang_vel_z', Parameter.Type.DOUBLE, 0.5)).get_parameter_value().double_value     # min, max 값 다른데 쓸 수 있음 1.2, 2.5 (하이리온 or 미니)
