...
private:
    ...
    void downsampleAndCluster(const sensor_msgs::msg::PointCloud2 &input_cloud_msg, sensor_msgs::msg::PointCloud2 &output_cloud_msg) {
        ...
        pcl::PointCloud<pcl::PointXYZ>::Ptr dense_projected = project_to_max_z(dense_cloud);
        pcl::PointCloud<pcl::PointXYZ>::Ptr sparse_projected = project_to_max_z(sparse_cloud);
    ...
    pcl::PointCloud<pcl::PointXYZ>::Ptr project_to_max_z(const pcl::PointCloud<pcl::PointXYZ>::Ptr& cloud) {
        std::map<std::pair<float, float>, float> max_z_points;
        pcl::PointCloud<pcl::PointXYZ>::Ptr projected_cloud(new pcl::PointCloud<pcl::PointXYZ>);

        for (const auto& point : cloud->points) {
            auto xy = std::make_pair(round(point.x * 100) / 100, round(point.y * 100) / 100);
            if (max_z_points.find(xy) == max_z_points.end() || max_z_points[xy] < point.z) {
